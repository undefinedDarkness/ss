...
return function (search, force)
	...
	while (index < #lines) do
		...
		if not current then
			...
			local icon = Gtk.Image.new_from_gicon(Gio.content_type_get_icon(Gio.content_type_guess(lines[index])), Gtk.IconSize.DND)
			icon:set_pixel_size(32)
			
			local title = Gtk.HBox {homogeneous= false, spacing = 8}
			title:pack_start(icon, false, false, 0)
			title:pack_start(require('ui.util').class(Gtk.Label { 
					label = lines[index]:gsub(home, "~"),
					xalign = 0,
				}, 'file-title'), false, false, 0)
